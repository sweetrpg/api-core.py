...
class APIData(BaseDataLayer):
    ...
    def __init__(self, kwargs):
        ...
        for model_type, model_info in self.model_info.items():
            __logger.info("Adding repository for type %s...", model_type)
    ...
    def create_object(self, model: BaseModel, view_kwargs: dict) -> Document:
        ...
        __logger.debug("data (%s): %s, view_kwargs: %s", model, type(model), view_kwargs)

        self.before_create_object(model, view_kwargs)

        data = model.to_dict()
        __logger.debug("data: %s", data)

        try:
            repo = self.repos[self.type]
            __logger.debug("repo: %s", repo)
            doc = repo.create(data)
            __logger.info("Document created: %s", doc)
            model_class = self.model_info[self.type]["model"]
            __logger.debug("model_class: %s", model_class)
            new_model = to_model(doc, model_class)
            __logger.debug("new_model: %s", new_model)
    ...
    def get_object(self, view_kwargs, qs=None):
        ...
        :return DeclarativeMeta: an object
        """
        __logger.debug("view_kwargs: %s, qs: %s", view_kwargs, qs)
        ...
        __logger.info("Looking up record for ID '%s'...", record_id)
        repo = self.repos[self.type]
        __logger.debug("repo: %s", repo)
        try:
            record = repo.get(record_id)
            __logger.debug("record: %s", record)
            if record is None:
    ...
    def get_collection(self, qs, view_kwargs, filters=None):
        ...
        """
        __logger.debug("qs: %s, view_kwargs: %s, filters: %s", qs, view_kwargs, filters)
        __logger.debug("querystring: %s", qs.querystring)
        __logger.debug(
    ...
    def update_object(self, obj, data, view_kwargs):
        ...
        """
        __logger.debug("obj: %s, data: %s, view_kwargs: %s", obj, data, view_kwargs)
        ...
        try:
            ...
            __logger.debug("updated_record: %s", updated_record)
        except:
    ...
    def delete_object(self, obj, view_kwargs):
        ...
        :param dict view_kwargs: kwargs from the resource view
        """
        __logger.debug("obj: %s, view_kwargs: %s", obj, view_kwargs)
        ...
        try:
            ...
            __logger.debug("is_deleted: %s", is_deleted)
        except:
            raise ObjectNotFound(f'Unable to delete {self.type} record for ID {view_kwargs["id"]}')
    ...
    def query(self, qs, view_kwargs):
        ...
        """
        __logger.debug("qs: %s, view_kwargs: %s", qs, view_kwargs)
    ...
    def paginate_query(self, query, paginate_info):
        ...
        """
        __logger.debug("query: %s, paginate_info: %s", query, paginate_info)
    ...
    def _convert_properties(self, obj):
        ...
        for p in date_properties + id_properties:
            __logger.debug("p (%s): %s", type(p), p)

            try:
                property_value = obj.get(p) or getattr(obj, p)
            except:
                __logger.debug("could not get property value '%s'; skipping", p)
                continue

            __logger.debug("property_value: %s", property_value)
            if property_value is None:
                continue

            __logger.debug("initializing new_property_value with current value")
            new_property_value = property_value

            if p in date_properties:
                __logger.debug("converting date property: %s, value: %s", p, property_value)
                new_property_value = to_datetime(property_value)
            elif p in id_properties:
                __logger.debug("converting ID property: %s, value: %s", p, property_value)
                if isinstance(property_value, dict):
                    __logger.debug("converting dictionary with possible $oid key")
                    new_property_value = property_value["$oid"]
                else:
                    __logger.debug("coercing property value into string from %s", type(property_value))
                    new_property_value = str(property_value)

                __logger.debug("new_property_value: %s", new_property_value)
                if p == "_id":
                    __logger.debug("deleting old value with key '%s'", p)
                    del obj[p]
                    __logger.debug("changing key of ID value '%s'", p)
                    p = "id"

            if isinstance(obj, dict):
                __logger.debug("setting new dictionary value '%s' in dict %s: %s", p, obj, new_property_value)
                obj[p] = new_property_value
            else:
                __logger.debug("setting new attribute value '%s' in object %s: %s", p, obj, new_property_value)
                setattr(obj, p, new_property_value)

        __logger.debug("converted object: %s", obj)
        return obj
    ...
    def _populate_object(self, obj, properties: dict):
        ...
        """
        __logger.debug("obj: %s, properties: %s", obj, properties)

        for property_name, property_type in properties.items():
            __logger.debug("property_name: %s, property_type: %s", property_name, property_type)
            ...
            if isinstance(property_value, str):
                __logger.debug("property_value is a string")

                new_property_value = self.repos[property_type].get(property_value)
                # __logger.info("new_value: %s", new_value)
                __logger.debug("new_property_value: %s", new_property_value)

                setattr(obj, property_name, new_property_value)

            if isinstance(property_value, list):
                __logger.debug("property_value is a list")

                new_property_value = []
                for list_value in property_value:
                    __logger.debug("list_value: %s", list_value)
                    ...
                    # __logger.debug("new_obj: %s", new_obj)
                    # new_value = json.loads(new_obj.to_json())
                    # __logger.debug("new_value: %s", new_value)
                __logger.debug("new_property_value: %s", new_property_value)
    ...
    def before_create_object(self, data, view_kwargs):
        ...
        :param dict view_kwargs: kwargs from the resource view
        """
        __logger.debug("data: %s, view_kwargs: %s", data, view_kwargs)
    ...
    def after_create_object(self, obj, data, view_kwargs):
        ...
        :param dict view_kwargs: kwargs from the resource view
        """
        __logger.debug("%s, data: %s, view_kwargs: %s", obj, data, view_kwargs)

    def before_get_object(self, view_kwargs):
        """Make work before to retrieve an object
        :param dict view_kwargs: kwargs from the resource view
        """
        __logger.debug("view_kwargs: %s", view_kwargs)
    ...
    def after_get_object(self, obj, view_kwargs):
        ...
        __logger.debug("this_model: %s", this_model)
        properties = this_model.get("properties", {})
        __logger.debug("properties: %s", properties)

        data = json.loads(obj.to_json())
        __logger.debug("data: %s", data)
        converted_data = self._convert_properties(data)
        __logger.debug("converted_data: %s", converted_data)
    ...
    def after_get_collection(self, collection, qs, view_kwargs):
        ...
        :param dict view_kwargs: kwargs from the resource view
        """
        __logger.debug("collection: %s, qs: %s, view_kwargs: %s", collection, qs, view_kwargs)
    ...
    def before_create_relationship(self, json_data, relationship_field, related_id_field, view_kwargs):
        ...
        :return boolean: True if relationship have changed else False
        """
        __logger.debug("relationship_field: %s, related_id_field: %s, view_kwargs: %s", relationship_field, related_id_field, view_kwargs)
